...
|---|---|---|---|---|
| C9 | `grantWriteSecureSettings()` | `pm grant 'io.github.openquesttuner' android.permission.WRITE_SECURE_SETTINGS` | aucune | code de sortie 0 **et** permission relue comme détenue par l'appli (`WirelessDebuggingSwitch.hasWriteRight()`) |
...
| C12 | `restoreAuthorizationExpiry(ms)` | `settings put global adb_allowed_connection_time <ms>` | `ms` : entier `Long` strictement positif, lu dans `expiry_original` | code de sortie 0 |
...
- **C11 à C13 ne visent qu'une clé**, `adb_allowed_connection_time`, écrite en constante. Le seul
  paramètre, `ms` de C12, est un entier positif formaté en décimal : `require(ms > 0)`, sinon
  `IllegalArgumentException` avant tout envoi. Aucune autre clé de `settings` n'est modifiable par
...
1. C9 ;
2. relecture de la permission (`hasWriteRight()`) ;
3. si elle est détenue : `auto_reconnect = true`, résultat `ENABLED` ; sinon, résultat
   `UNAVAILABLE`, et l'option reste inactive.

### Désactiver (FR-013, FR-014)

1. `auto_reconnect = false`, `revoke_pending = true` (écrits **avant** C10) ;
2. si l'appli est `Connected` : C10, puis `revoke_pending = false`, résultat `REVOKED` ;
3. sinon : résultat `REVOKE_PENDING`.
...
- dans tous les cas : `revoke_pending = false`.

### Ne jamais faire expirer les autorisations (FR-021, FR-024)

Seulement quand l'appli est `Connected`, après confirmation, et si le délai lu n'est pas `0` :
